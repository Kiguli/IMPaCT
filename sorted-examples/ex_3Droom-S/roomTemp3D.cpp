...
// States
const vec ss_lb = {19, 19,19};
const vec ss_ub = {21,21,21};
const vec ss_eta = {0.1, 0.1, 0.1};
// Inputs
const vec is_lb = {0,0};
...
const vec sigma = {sqrt(1/50), sqrt(1/50), sqrt(1/50)};

//dynamics - 2 parameters
const auto dynamics = [](const vec& x, const vec& u) -> vec {
    vec xx(dim_x);
    float eta = 0.20f, beta = 0.022f, gamma = 0.05f, a = 1.0f - 2.0f*0.20f - 0.022f, T_h = 50.0f, T_e = -1.0f;
    xx(0) = (a - gamma*u(0))*x(0) + eta*(x(1) + x(2)) + gamma*T_h*u(0) + beta*T_e;
    xx(1) = a*x(1) + eta*(x(0) + x(2)) + beta*T_e;
    xx(2) = (a - gamma*u(1))*x(2) + eta*(x(0) + x(1)) + gamma*T_h*u(1) + beta*T_e;
    return xx;
};
...
int main() {
    ...
    mdp.infiniteHorizonSafeControllerSorted(false);
